...
void drawMapElements(DataHolder* in, bool editor)
{
    ...
    if (!editor)
    {
        ...
        {
            DrawTexturePro(in->textures.tileTexture,in->lists.tiles.tileCrops.at(153),toRayLibRectangle((*i)->getPosition()*Vec2D(48,48)+Vec2D(50,50),Vec2D(48,48)),Vec2D(24,24),0,LIGHTGRAY);
            DrawTexturePro(in->textures.tileTexture,in->lists.tiles.tileCrops.at((*i)->getTexture()),toRayLibRectangle((*i)->getPosition()*Vec2D(48,48)+Vec2D(50,50),Vec2D(80,80)),Vec2D(40,40),(*i)->getRotation()*RAD2DEG+90.0f,GetColor((*i)->getColor()));
        } 
